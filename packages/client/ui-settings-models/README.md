---
description: "Models settings and product-onboarding plugin for the dsh web client: provider rows, API-key management, model lists, and the first-run provider picker."
...
After the versioned notice step completes, the provider step projects first-run readiness from the same joined snapshot. ANY provider the user can already reach ends it without rendering. A user with none sees a searchable list of every configurable directory row plus **Add a custom provider**; official DeepSeek is one optional row, not a required key field. Choosing a row opens that provider's editor; Configure later completes only this coordinator pass. A failed join, read-only deployment, credential-service failure, or directory with no configurable address and no `llm-pi-ai` namespace completes the step without rendering — Models remains the diagnostic surface.
