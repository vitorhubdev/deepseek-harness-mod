...
describe.skipIf(MODE === 'record')('web e2e: first-run DeepSeek credential setup', () => {
  ...
  it('stores a key write-only and observes configured state without restarting', async () => {
    ...
    const picker = page.getByRole('dialog', { name: '选择一个提供方开始使用' })
    await picker.waitFor({ timeout: 15_000 })
    await picker.getByRole('button', { name: 'DeepSeek', exact: true }).click()
    const credentialStep = page.getByRole('dialog', { name: 'DeepSeek' })
    ...
    expect(await page.getByRole('dialog', { name: '选择一个提供方开始使用' }).count()).toBe(0)
    ...
    await page.addInitScript(() => {
      ...
      setInterval(() => {
        if (document.querySelector(
          ...
          + '[role="dialog"][aria-label="选择一个提供方开始使用"]',
        ) !== null) {
